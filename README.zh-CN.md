...
[![platform](https://img.shields.io/badge/platform-Windows%2010%20%7C%2011-0078D4?logo=windows&logoColor=white)](#快速开始)
[![powershell](https://img.shields.io/badge/PowerShell-5.1%2B-5391FE?logo=powershell&logoColor=white)](#快速开始)
[![dependencies](https://img.shields.io/badge/dependencies-none-brightgreen)](#不联网不上报)
...
| **界面中英双语** | **装的时候自动按系统语言定好**（中文系统 = 中文，其它 = English），也可以在安装窗口里按 `1` / `2` 手选；装完随时能在设置界面里切换。两边的文案都是完整写的，不是机翻占位。 |
...
2. 双击 **`install.bat`**
   > 第一次安装会问一句界面语言：`[1] English` / `[2] 中文`。
   > **什么都不按也行** —— 10 秒后按你的系统语言自动继续（中文系统就是中文，不会因为手慢变成英文）。
   > 装完还能随时在设置界面里改。
3. 桌面上会出现一个「F9开工」图标。装完就有配置文件了，能直接开工
