...
![主界面](screenshots/skin-1-minimal.png)
...
| | |
|---|---|
| **应用和设置二合一** | 桌面只有**一个**图标。双击打开的就是控制面板：清单、开关、皮肤、彩蛋全在一个窗口里改，改完点【保存并生效】。 |
...
| **一键收工** | 按 `Ctrl+Alt+Q`（或双击桌面「F9收工」图标）弹出**关机 / 重启 / 睡眠**三选一，**默认等 60 秒**才真的执行 —— 这段时间里点【取消】、按 Esc、点右上角那个 × 都能立刻停下，什么都不会发生。关机刻意**不加 `/f`**，行为跟开始菜单里的「关机」完全一致（有没保存的文档时 Windows 会照常问你）。想让某个动作别出现，在控制面板里取消勾选即可。 |
...
2. 双击 **`install.bat`**
3. 桌面上会出现一个「F9开工」图标，双击它打开控制面板
4. 在 ① 里勾选要打开的软件（点【添加软件】可以浏览选择），在 ② 里粘贴要打开的网址
5. 点右下角 **【保存并生效】**
6. 之后每天：**按一下 `F9` 或 `Ctrl+Alt+W`** —— 开工。
7. 下班前：**按一下 `Ctrl+Alt+Q`**（或双击桌面「F9收工」）—— 关机 / 重启 / 睡眠，带倒计时，随时能反悔。

> 想让桌面图标**双击就直接开工**（像一开始那样一步到位）？
> 在控制面板的「③ 其它设置」里把 **双击桌面图标** 改成「直接开工」，保存即可。
...
下班的时候不用再去找开始菜单 —— 按 `Ctrl+Alt+Q`，或者双击桌面上的「F9收工」图标：
...
**两种入口，互相独立**：`Ctrl+Alt+Q` 走的是后台那个常驻进程；桌面「F9收工」图标是**另起一个进程**去弹窗，
所以**后台没在跑（甚至被关掉了）这个图标照样能用**。

不想要了怎么办：

- 只想去掉快捷键 → 控制面板里把「收工快捷键」那一格**清空**，保存并生效。
- 连桌面图标也想去掉 → 直接删掉那个图标即可（不影响任何其它功能）。
- 只想留一两个动作 → 目前三个动作是固定显示的（用户明确说了三个都要）；
...
├── art/                   四套皮肤的单色几何图案（256×256 透明 PNG）
├── run-app.vbs            桌面「F9开工」图标靠它静默启动（零黑框），别手改
├── run-quit.vbs           桌面「F9收工」图标靠它静默启动（零黑框），别手改
├── skin_*_app.ico         各皮肤对应的桌面图标
├── skin_*_setup.ico       各皮肤对应的设置图标
├── quit.ico / skin_*_quit.ico   「F9收工」图标的图案
│
...
```jsonc
{
  ...
  "voiceVolume": 80,
  "lang": "zh",                   // zh / en
  "iconAction": "panel",          // panel=双击图标开面板 / run=直接开工
  "apps": [
