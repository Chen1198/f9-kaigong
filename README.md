...
桌面图标双击出来的是这个 **启动台**：点一下中间那个大圆环就开工（鼠标移上去圆环会变亮，
按下去有回弹和一声电子音）。圆环下面那行写的是你今天要开几项。**界面上就只有"开工"这一件事**，
别的都不摆在脸上。
...
| | |
|---|---|
| **一个图标，一个动作** | 桌面只有**一个**「F9开工」图标。双击出来的是**启动台**——一个大圆环，点一下就开工。界面上的状态行会告诉你今天要开几项。 |
...
7. 之后每天：**按一下 `F9` 或 `Ctrl+Alt+W`** —— 开工。
   不想起手翻键盘？**双击桌面图标 → 点一下中间那个大圆环**，一样的效果。
...
├── art/                   皮肤装饰图 + 启动台的美术资源（cyber_*.png / cyber_launch.wav）
├── hub.ico                桌面「F9开工」图标的图案（赛博朋克圆环）
